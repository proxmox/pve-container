...
sub param_mapping {
    ...
    my $mapping = {
        'create_vm' => [
            PVE::CLIHandler::get_standard_mapping('pve-password'), 'ssh-public-keys',
        ],
    };
    ...
    parameters => {
        ...
        properties => {
            ...
            'keep-env' => {
                type => 'boolean',
                description =>
                    "Keep the current environment. This option will disabled by default with PVE 9."
...
__PACKAGE__->register_method({
    ...
    code => sub {
        ...
        my $do_fsck = sub {
            ...
            defined($conf->{$device})
                || die "cannot run command on non-existing mount point $device\n";
                ...
                eval {
                    PVE::LXC::Config->foreach_volume(
                        ...
                        sub {
                            ...
                            my $entry =
                                [$name, $mp->{volume}, $total, $used, $avail, $pc, $path];
                            push @list, $entry;
...
our $cmddef = {
    ...
    create =>
        ['PVE::API2::LXC', 'create_vm', ['vmid', 'ostemplate'], { node => $nodename }, $upid_exit],
    restore => [
        'PVE::API2::LXC',
        'create_vm',
        ['vmid', 'ostemplate'],
        { node => $nodename, restore => 1 },
        $upid_exit,
    ],
    destroy => ['PVE::API2::LXC', 'destroy_vm', ['vmid'], { node => $nodename }, $upid_exit],

    start =>
        ['PVE::API2::LXC::Status', 'vm_start', ['vmid'], { node => $nodename }, $upid_exit],
    suspend =>
        ['PVE::API2::LXC::Status', 'vm_suspend', ['vmid'], { node => $nodename }, $upid_exit],
    resume =>
        ['PVE::API2::LXC::Status', 'vm_resume', ['vmid'], { node => $nodename }, $upid_exit],
    shutdown =>
        ['PVE::API2::LXC::Status', 'vm_shutdown', ['vmid'], { node => $nodename }, $upid_exit],
    stop => ['PVE::API2::LXC::Status', 'vm_stop', ['vmid'], { node => $nodename }, $upid_exit],
    reboot =>
        ['PVE::API2::LXC::Status', 'vm_reboot', ['vmid'], { node => $nodename }, $upid_exit],

    clone =>
        ["PVE::API2::LXC", 'clone_vm', ['vmid', 'newid'], { node => $nodename }, $upid_exit],
    migrate =>
        ["PVE::API2::LXC", 'migrate_vm', ['vmid', 'target'], { node => $nodename }, $upid_exit],
    'move-volume' => [
        "PVE::API2::LXC",
        'move_volume',
        ['vmid', 'volume', 'storage', 'target-vmid', 'target-volume'],
        { node => $nodename },
        $upid_exit,
    ],
