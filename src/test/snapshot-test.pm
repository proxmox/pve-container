...
sub testcase_prepare {
    ...
    subtest "Preparing snapshot '$snapname' for vm '$vmid'" => sub {
        ...
        ok(
            test_file(
                "snapshot-expected/prepare/lxc/$vmid.conf",
                "snapshot-working/prepare/lxc/$vmid.conf",
            ),
            "config file correct",
        );
    };
...
sub testcase_commit {
    ...
    subtest "Committing snapshot '$snapname' for vm '$vmid'" => sub {
        plan tests => 2;
        $@ = undef;
        eval { PVE::LXC::Config->__snapshot_commit($vmid, $snapname); };
        is($@, $exp_err, "\$@ correct");
        ok(
            test_file(
                "snapshot-expected/commit/lxc/$vmid.conf",
                "snapshot-working/commit/lxc/$vmid.conf",
            ),
            "config file correct",
        );
    }
...
sub testcase_create {
    ...
    subtest "Creating snapshot '$snapname' for vm '$vmid'" => sub {
        ...
        $@ = undef;
        eval { PVE::LXC::Config->snapshot_create($vmid, $snapname, $save_vmstate, $comment); };
        is($@, $exp_err, "\$@ correct");
        ...
        ok(
            test_file(
                "snapshot-expected/create/lxc/$vmid.conf",
                "snapshot-working/create/lxc/$vmid.conf",
            ),
            "config file correct",
        );
    };
...
sub testcase_delete {
    ...
    subtest "Deleting snapshot '$snapname' of vm '$vmid'" => sub {
        ...
        $exp_vol_snap_delete = {} if !defined($exp_vol_snap_delete);
        $@ = undef;
        eval { PVE::LXC::Config->snapshot_delete($vmid, $snapname, $force); };
        is($@, $exp_err, "\$@ correct");
        is_deeply(
            $vol_snapshot_delete,
            $exp_vol_snap_delete,
        ...
        ok(
            test_file(
                "snapshot-expected/delete/lxc/$vmid.conf",
                "snapshot-working/delete/lxc/$vmid.conf",
            ),
            "config file correct",
        );
    };
...
sub testcase_rollback {
    ...
    subtest "Rolling back to snapshot '$snapname' of vm '$vmid'" => sub {
        ...
        $@ = undef;
        eval { PVE::LXC::Config->snapshot_rollback($vmid, $snapname); };
        is($@, $exp_err, "\$@ correct");
        is_deeply(
            $vol_snapshot_rollback,
        ...
        ok(
            test_file(
                "snapshot-expected/rollback/lxc/$vmid.conf",
                "snapshot-working/rollback/lxc/$vmid.conf",
            ),
            "config file correct",
        );
    };
...
printf(
    "\tconfig(), volume_snapshot(), volume_snapshot_delete(), volume_snapshot_rollback(), volume_rollback_is_possible() and volume_snapshot_needs_fsfreeze() mocked\n"
);
